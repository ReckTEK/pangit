#!/bin/sh
# Generated Gitea 1.26.4 sandbox bootstrap. Do not edit.
set -eu
...
ssh-keygen -q -t ed25519 -N '' -C 'gitea-sandbox-1.26.4' -f "$results/id_ed25519"
public_key="$(cat "$results/id_ed25519.pub")"
wget -qO "$results/ssh-key.json" \
  --header="Authorization: token $token" \
  --header='Content-Type: application/json' \
  --post-data="{\"title\":\"sandbox-1.26.4\",\"key\":\"$public_key\"}" \
