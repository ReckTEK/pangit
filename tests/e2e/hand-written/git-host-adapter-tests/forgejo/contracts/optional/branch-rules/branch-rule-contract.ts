...
>(
  ...
  const passed = await t.step("shared-capability/branch-rules", async () => {
    ...
    await prove("repository.branchRules.create.unsupportedForcePush", [], async () => {
      try {
        await rules.create({ name: "unsupported", forcePushAllowed: true });
      } catch (error) {
        if (error instanceof CapabilityUnavailableError) return;
        throw error;
      }
      throw new Error("unsupported force-push permission accepted");
    });
    ...
    const nativeRuleName = await prove(
      "branchRule.native.forgejo",
      [],
      () =>
        updated.native.forgejo(({ configuredRule }) => Promise.resolve(configuredRule.rule_name)),
    );
