...
import type { PullRequestReviewContractFixtures } from "../../../../../fluent-api-contracts/optional/pull-request-reviews/pull-request-review-contract-fixtures.ts";
...
>(
  ...
  const passed = await t.step("shared-capability/pull-request-reviews", async () => {
    ...
    const nativeState = await prove(
      "pullRequestReview.native.forgejo",
      [],
      () => pending.native.forgejo(({ review }) => Promise.resolve(review.state)),
    );
