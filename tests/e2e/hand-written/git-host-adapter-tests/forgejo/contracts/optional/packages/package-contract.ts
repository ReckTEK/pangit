...
>(
  ...
  const passed = await t.step("shared-capability/packages", async () => {
    ...
    const nativeName = await prove(
      "packageVersion.native.forgejo",
      [],
      () => direct.native.forgejo(({ package: nativePackage }) => nativePackage.name),
    );
    assert(nativeName === coordinates.name, "Package native payload was not retained");
    const nativeFileName = await prove(
      "packageFile.native.forgejo",
      [],
      () => file.native.forgejo(({ packageFile }) => packageFile.name),
    );
    ...
    assertions.push(
      "version deletion is direct; whole-package deletion snapshots exact version identities first",
    );
  });
