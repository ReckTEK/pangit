...
import type { ReleaseContractFixtures } from "../../../../../fluent-api-contracts/optional/releases/release-contract-fixtures.ts";
...
>(
  ...
  const passed = await t.step("shared-capability/releases", async () => {
    ...
    const nativeTag = await prove(
      "release.native.forgejo",
      [],
      () => updated.native.forgejo(({ release }) => release.tag_name),
    );
    assert(nativeTag === input.fixtures.tagName, "Release native payload was not retained");
    const nativeAssetName = await prove(
      "releaseAsset.native.forgejo",
      [],
      () => renamed.native.forgejo(({ releaseAsset }) => releaseAsset.name),
    );
