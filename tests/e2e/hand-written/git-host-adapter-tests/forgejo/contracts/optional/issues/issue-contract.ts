...
import type { IssueContractFixtures } from "../../../../../fluent-api-contracts/optional/issues/issue-contract-fixtures.ts";
...
>(
  ...
  const passed = await t.step("shared-capability/issues", async () => {
    ...
    const nativeTitle = await prove(
      "issue.native.forgejo",
      [],
      () => updated.native.forgejo(({ issue }) => issue.title),
    );
    assert(nativeTitle === updated.title, "Issue native payload was not retained");
    const nativeCommentBody = await prove(
      "issueComment.native.forgejo",
      [],
      () => editedComment.native.forgejo(({ issueComment }) => issueComment.body),
    );
    assert(
