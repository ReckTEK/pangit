...
import type { CommitStatusContractFixtures } from "../../../../fluent-api-contracts/commit-statuses/commit-status-contract-fixtures.ts";
...
>(
  ...
  const passed = await t.step("core/commit-statuses", async () => {
    ...
    const success = await prove(
      ...
      ["repoGetSingleCommit", "repoCreateStatus"],
      () =>
    ...
    const branchStatuses = await prove(
      ...
      ["repoGetSingleCommit", "repoListStatusesByRef"],
    ...
    const tagStatus = await prove(
      ...
      ["repoGetSingleCommit", "repoGetCombinedStatusByRef"],
