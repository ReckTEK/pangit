...
>(
  ...
  const passed = await t.step("core/content-reads", async () => {
    ...
    await prove("readText.invalidUtf8", ["repoGetContents"], () =>
      expectReadError(
    ...
    for (const path of [input.fixtures.invalidJsonPath, input.fixtures.emptyPath]) {
      await prove(`readJson.invalid:${path}`, ["repoGetContents"], () =>
        expectReadError(
    ...
    for (const path of [input.fixtures.image.path, input.fixtures.image.extensionlessPath]) {
      const image = await prove(
        `readBlob.image:${path}`,
        ["repoGetContents", "repoGetRawFile"],
    ...
    assertions.push(
      "batch reads deduplicate paths with bounded concurrent requests and restore caller order",
    );
    ...
    await prove("readBlob.directory", ["repoGetContents"], () =>
      expectReadError(
        () =>
          repository.content.readBlob(input.fixtures.nestedDirectory, { ref: input.fixtures.ref }),
        "not-a-file",
      ));
    await prove("readText.directory", ["repoGetContents"], () =>
      expectReadError(
        () =>
    ...
    const collapsed = await prove(
      ...
      ["repoGetContents", "repoGetContents", "repoGetContents"],
      () =>
    ...
    const metadata = await prove(
      ...
      ["repoGetSingleCommit", "repoGetContents", "repoGetContents"],
      () =>
    ...
    const internalSubmodule = await prove(
      ...
      ["repoGetContents", "repoGetContentsList"],
      () =>
    ...
    assertions.push(
      "linked reads are metadata-only by default; explicit dereference stays inside Forgejo",
    );
