...
import type { FileChangeContractFixtures } from "../../../../fluent-api-contracts/content/content-contract-fixtures.ts";
...
>(
  ...
  const passed = await t.step("core/file-change-commits", async () => {
    ...
    const committed = await prove(
      ...
      ["repoGetContents", "repoGetContents", "repoGetContents", "repoChangeFiles"],
      () =>
    ...
    assertions.push(
      "mixed create/update/move/delete is one commit after bounded source reads",
    );
    ...
    assert(staleGuardConflict, "A stale Forgejo SHA guard overwrote content");
