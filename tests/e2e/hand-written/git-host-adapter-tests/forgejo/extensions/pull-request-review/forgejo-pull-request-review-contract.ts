...
import type { ForgejoPullRequestReviewFixtures } from "./forgejo-pull-request-review-fixtures.ts";

export type ForgejoPullRequestReviewContractInput = {
  readonly version: ProviderVersion<"forgejo">;
  readonly apiUrl: string;
  readonly fixtures: ForgejoPullRequestReviewFixtures;
};
...
/** Prove Forgejo review events and grouped old/new inline coordinates through the fluent builder. */
export async function runForgejoPullRequestReviewContract(
  t: Deno.TestContext,
  input: ForgejoPullRequestReviewContractInput,
  ...
  const passed = await t.step("forgejo-extension/pull-request-review", async () => {
    ...
    const capture = await recorder.capture(() =>
      repository.pullRequests.reviews(pullRequest).create({
        body: "PanGit grouped Forgejo review",
    ...
    const proof = proveRequestSequence(
      "createPullRequestReview.forgejoExtension",
    ...
    assert(
      ...
      "Review extension did not preserve exact Forgejo event/position fields",
    );
    assert(proof.value.state === "pending", "Forgejo pending review state was not normalized");
    assert(Object.isFrozen(proof.value), "Forgejo review snapshot is mutable");
    assertions.push(
  ...
  return Object.freeze({
    id: "forgejo-extension/pull-request-review",
    passed,
