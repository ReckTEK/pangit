...
import type { ForgejoFileChangeCommitFixtures } from "./forgejo-file-change-commit-fixtures.ts";

export type ForgejoFileChangeCommitContractInput = {
  readonly version: ProviderVersion<"forgejo">;
  readonly apiUrl: string;
  readonly token: string;
  readonly fixtures: ForgejoFileChangeCommitFixtures;
};
...
/** Prove the typed Forgejo file-change extension without widening the common input. */
export async function runForgejoFileChangeCommitContract(
  t: Deno.TestContext,
  input: ForgejoFileChangeCommitContractInput,
  ...
  const passed = await t.step("forgejo-extension/file-change-commit", async () => {
    ...
    const capture = await recorder.capture(() =>
      repository.content.commitChanges({
        ...
        message: "PanGit Forgejo extension authorship",
        author: {
        ...
        changes: [{
          ...
          content: "Forgejo extension fixture\n",
        }],
      }).forgejo((context) => {
        callbackCount++;
    ...
    const proof = proveRequestSequence(
      "commitFileChanges.forgejoExtension",
      ["repoChangeFiles"],
      capture,
    );
    ...
    assert(
      ...
      "Forgejo author identity/date were not retained",
    );
    assert(
      ...
      "Forgejo committer identity/date were not retained",
    );
    assert(
      commit.message.includes("Signed-off-by: PanGit E2E Committer <committer@example.invalid>"),
      "Forgejo signoff was not present in the created commit",
    );
    assert(Object.isFrozen(commit), "Forgejo extension returned a mutable commit snapshot");
    assertions.push(
  ...
  return Object.freeze({
    id: "forgejo-extension/file-change-commit",
    passed,
