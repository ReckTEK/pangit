...
type ForgejoVersion = ProviderVersion<"forgejo">;
type ForgejoClient<TVersion extends ForgejoVersion> = RestClientTypeMap["forgejo"][TVersion];
...
/**
 * Owns the Forgejo-only setup needed to prove the universal authentication contract live.
 * Raw API setup and browser-form driving stay outside the behavior contract itself.
 */
export class ForgejoAuthenticationFixtureDriver<TVersion extends ForgejoVersion> {
  ...
  static async create<const TVersion extends ForgejoVersion>(input: {
    ...
    readonly timeoutMs: number;
  }): Promise<ForgejoAuthenticationFixtureDriver<TVersion>> {
    ...
    return new ForgejoAuthenticationFixtureDriver(input.version, client, input.timeoutMs);
  }
  ...
  async createFixtures(input: {
    ...
    const totpSession = new ForgejoWebSession(webRoot, this.#timeoutMs);
    await totpSession.login(totpUsername, totpPassword);
    const enrollment = await enrollTotp(totpSession);

    const callbackUrl = `http://pangit-e2e.invalid/oauth/callback?type=forgejo`;
    ...
    return Object.freeze({
      ...
      oauth: Object.freeze({
        ...
        authorize: async (authorizationUrl: URL) => {
          const session = new ForgejoWebSession(webRoot, this.#timeoutMs);
  ...
  async cleanup(): Promise<void> {
    ...
    if (errors.length > 0) {
      throw new Error(`Forgejo authentication fixture cleanup failed: ${errors.join("; ")}`);
    }
  }
  ...
  #requireOpen(): void {
    if (this.#closed) throw new Error("Forgejo authentication fixture driver is already closed");
  }
}
...
class ForgejoWebSession {
  ...
  async login(username: string, password: string): Promise<void> {
    ...
    if (!isRedirect(response.status)) {
      throw new Error(`Forgejo web login failed with HTTP ${response.status}`);
    }
  }
  ...
  async follow(url: URL, init: RequestInit = {}): Promise<Response> {
    ...
    for (let redirects = 0; redirects <= 8; redirects++) {
      ...
      if (location === null) throw new Error("Forgejo redirect omitted Location");
      current = new URL(location, current);
      method = "GET";
      body = undefined;
    }
    throw new Error("Forgejo web flow exceeded its redirect limit");
  }
...
async function enrollTotp(
  ...
  if (!page.ok) throw new Error(`Forgejo TOTP enrollment page failed with HTTP ${page.status}`);
  ...
  if (secretMatch === null) throw new Error("Forgejo TOTP enrollment secret is missing");
  ...
  if (!isRedirect(response.status)) {
    throw new Error(`Forgejo TOTP enrollment failed with HTTP ${response.status}`);
  }
...
async function authorizeOAuth(
  ...
  if (!grantPage.ok) {
    throw new Error(`Forgejo OAuth grant page failed with HTTP ${grantPage.status}`);
  }
  ...
  if (!isRedirect(response.status)) {
    throw new Error(`Forgejo OAuth grant failed with HTTP ${response.status}`);
  }
  const location = response.headers.get("location");
  if (location === null) throw new Error("Forgejo OAuth grant omitted its callback redirect");
  const callback = new URL(location, grant.action);
  const expected = new URL(callbackUrl);
  if (callback.origin !== expected.origin || callback.pathname !== expected.pathname) {
    throw new Error("Forgejo OAuth grant redirected to an unexpected callback");
  }
...
function parseForm(html: string, pageUrl: string, actionFragment: string): Form {
  ...
  throw new Error(`Forgejo form ${actionFragment} is missing`);
}
...
function requiredString(value: unknown, label: string): string {
  if (typeof value !== "string" || value.trim().length === 0) {
    throw new Error(`Forgejo ${label} is missing`);
  }
  return value;
}
