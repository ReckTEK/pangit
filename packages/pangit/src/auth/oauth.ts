import type { ClientOptions } from "../client/core.ts";
...
import { AuthAdapterNotImplementedError, OAuthCallbackError } from "./errors.ts";
...
> implements Login<TProvider, TVersion> {
  ...
  async authorize(
    ...
    const fetcher = this.#clientOptions.fetch ??
    ...
    const currentUser = await client.userGetCurrent();
