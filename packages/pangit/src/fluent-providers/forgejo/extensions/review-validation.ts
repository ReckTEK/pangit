...
export function validateForgejoCreateReviewExtension(
  extension: Readonly<ForgejoCreatePullRequestReviewExtension>,
  context: ValidationErrorContext,
): void {
  if (
    extension.event !== undefined &&
    !FORGEJO_REVIEW_EVENTS.includes(extension.event)
  ) {
  ...
  if (extension.comments.length === 0) {
    throw new ValidationError("Forgejo review comments cannot be empty", context);
  }
