...
import {
  ForgejoAdapterContext,
  type ForgejoAdapterOptions,
} from "./transport/ForgejoAdapterContext.ts";
import { createForgejoClientNative } from "./native/ForgejoClientNative.ts";
import { forgejoExtensions } from "./extensions/runtime.ts";
import { getForgejoUnsupportedOptionalCapabilities } from "./unsupported-capabilities.ts";
...
export function createForgejoAdapter<V extends ForgejoVersion>(
  version: V,
  options: ForgejoAdapterOptions,
  context = new ForgejoAdapterContext(version, options),
): Adapter<V> {
  const withContext = (authorized: ForgejoAdapterContext<V>) =>
    createForgejoAdapter(version, { baseUrl: authorized.webBaseUrl() }, authorized);
  return Object.freeze({
    provider: "forgejo",
    version,
    extensions: forgejoExtensions,
    unsupportedOptionalCapabilities: getForgejoUnsupportedOptionalCapabilities(version),
    native: createForgejoClientNative(() => context.client()),
