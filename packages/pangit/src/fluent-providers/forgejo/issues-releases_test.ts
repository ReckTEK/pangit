...
import { getForgejoCurrentUserProfile } from "./current-user-profile/mod.ts";
import {
  createForgejoIssueComment,
  deleteForgejoIssueComment,
  getForgejoIssueComment,
  listForgejoIssueComments,
  listForgejoIssues,
  setForgejoIssueState,
  updateForgejoIssue,
...
import {
  ...
  getForgejoRelease,
  getForgejoReleaseAsset,
  getForgejoReleaseByTag,
  listForgejoReleaseAssets,
  listForgejoReleases,
...
Deno.test("Forgejo issue-comment and release-asset lifecycles stay direct and bounded", async () => {
  ...
  const repository = fixtureRepository("16.0.3");
  const issue = (await listForgejoIssues(
    new ForgejoAdapterContext("16.0.3", {
      baseUrl: "https://forgejo.example.invalid/api/v1",
  ...
  const closed = await setForgejoIssueState(context, repository, issue, "closed");
  assertEquals(closed.state, "closed", "issue state normalization changed");
  const commentPage = await listForgejoIssueComments(context, repository, issue, { limit: 5 });
  assertEquals(
  ...
  const directComment = await getForgejoIssueComment(context, repository, "9");
  const createdComment = await createForgejoIssueComment(
    context,
    repository,
    issue,
    { body: "created" },
  );
  const updatedComment = await updateForgejoIssueComment(
    context,
    repository,
    createdComment,
    { body: "updated" },
  ...
  await deleteForgejoIssueComment(context, repository, directComment);

  const releasePage = await listForgejoReleases(context, repository, { limit: 4 });
  assertEquals(releasePage.items.length, 1, "release list normalization changed");
  const release = await getForgejoRelease(context, repository, "31");
  assertEquals(
    (await getForgejoReleaseByTag(context, repository, "v1.0.0")).id,
    "31",
    "release tag lookup changed",
  );
  assertEquals(
    (await updateForgejoRelease(context, repository, release, { name: "Renamed" })).name,
    "Renamed",
    "release update normalization changed",
  );
  const assets = await listForgejoReleaseAssets(context, repository, release, { maxItems: 2 });
  const asset = assets[0];
  assertEquals(
    (await getForgejoReleaseAsset(context, repository, release, "41")).name,
    "artifact.txt",
    "asset direct lookup changed",
  );
  assertEquals(
    (await updateForgejoReleaseAsset(
      context,
      repository,
      release,
      asset,
  ...
  await deleteForgejoReleaseAsset(context, repository, release, asset);
  await deleteForgejoRelease(context, repository, release);
...
for (const version of ["15.0.7", "16.0.3"] as const) {
  Deno.test(`Forgejo ${version} optional profile, issues, releases, and assets use direct operations`, async () => {
    const requests: Request[] = [];
    ...
    const context = new ForgejoAdapterContext(version, {
      ...
      fetch: async (input, init) => {
        ...
        if (
          ...
          return jsonResponse({ ...issuePayload(), body: "changed" }, 201);
        }
        if (
          request.method === "POST" &&
    ...
    const profile = await getForgejoCurrentUserProfile(context);
    assertEquals(profile.username, "fixture", "current-user normalization changed");
    const issues = await listForgejoIssues(context, repository, { limit: 10 });
    assertEquals(issues.items.length, 1, "issue page normalization changed");
    const updated = await updateForgejoIssue(
      context,
      repository,
      issues.items[0],
      { description: "changed" },
    ...
    assertEquals(
      issueEdits,
      [{ body: "changed" }],
      "issue update body changed",
    );

    const release = await createForgejoRelease(context, repository, {
      tagName: "v1.0.0",
      name: "Version 1",
      target: "main",
    });
    const asset = await uploadForgejoReleaseAsset(
      context,
      repository,
      release,
    ...
    const nativeId = await release.native.forgejo(({ release: native }) => native.id);
...
function commentPayload(id: number, issueNumber: number) {
  return {
    id,
    body: "comment",
    issue_url: `https://forgejo.example.invalid/api/v1/repos/acme/project/issues/${issueNumber}`,
    user: { login: "fixture" },
  };
}
