...
import {
  authorizeForgejoBasic,
  authorizeForgejoToken,
  beginForgejoOAuth,
...
Deno.test("Forgejo PAT authorization performs one direct verification with the token scheme", async () => {
  ...
  const authorized = await authorizeForgejoToken(context, { token: "secret-token" });
...
Deno.test("Forgejo Basic authorization is UTF-8 safe and carries the optional OTP once", async () => {
  const requests: Request[] = [];
  const context = new ForgejoAdapterContext("15.0.7", {
    baseUrl: "https://forgejo.example.invalid/api/v1",
    fetch: (input, init) => {
...
Deno.test("Forgejo authorization validates credentials before any request and does not leak secrets", async () => {
  let requests = 0;
  const context = new ForgejoAdapterContext("16.0.3", {
    baseUrl: "https://forgejo.example.invalid/api/v1",
    fetch: () => {
  ...
  for (
    const [operation, execute] of [
      ["authorizeToken", () => authorizeForgejoToken(context, { token: "   " })],
      [
        "authorizeToken",
        () => authorizeForgejoToken(context, { token: "secret", tokenType: "not a scheme" }),
      ],
      [
        "authorizeBasic",
        () => authorizeForgejoBasic(context, { username: "a:b", password: "secret" }),
      ],
      ["authorizeBasic", () => authorizeForgejoBasic(context, { username: "user", password: "" })],
      ["authorizeBasic", () =>
        authorizeForgejoBasic(context, {
          username: "user",
          password: "secret",
    ...
    assertEquals(error.provider, "forgejo", "authorization validation provider changed");
    assertEquals(error.version, "16.0.3", "authorization validation version changed");
  }
  assertEquals(requests, 0, "invalid local credentials reached the provider");

  const error = await assertRejects(
    () => authorizeForgejoToken(context, { token: "do-not-leak-this" }),
    AuthenticationError,
  );
...
Deno.test("Forgejo OAuth begin creates the provider URL locally", () => {
  const context = new ForgejoAdapterContext("16.0.3", {
    baseUrl: "https://git.example.invalid/custom/api/v1",
    webBaseUrl: "https://login.example.invalid/forgejo/",
  });
  const result = beginForgejoOAuth(context, {
    clientId: "client-id",
    callbackUrl: new URL("https://app.example.invalid/oauth?type=forgejo"),
  ...
  assertEquals(
    ...
    {
      client_id: "client-id",
      redirect_uri: "https://app.example.invalid/oauth?type=forgejo",
      response_type: "code",
      state: "state-value",
...
Deno.test("Forgejo OAuth exchange sends one sanitized form request and normalizes metadata", async () => {
  const requests: Request[] = [];
  const context = new ForgejoAdapterContext("16.0.3", {
    baseUrl: "https://forgejo.example.invalid/api/v1?unsafe=default",
  ...
  const token = await exchangeForgejoOAuthCode(context, {
    clientId: "client-id",
  ...
  assertEquals(
    requests[0].url,
    "https://forgejo.example.invalid/login/oauth/access_token",
...
Deno.test("Forgejo OAuth exchange rejects malformed success payloads", async () => {
  const context = new ForgejoAdapterContext("16.0.3", {
    baseUrl: "https://forgejo.example.invalid/api/v1",
  ...
  await assertRejects(
    () =>
      exchangeForgejoOAuthCode(context, {
        clientId: "client-id",
        callbackUrl: new URL("https://app.example.invalid/oauth?type=forgejo"),
        code: "authorization-code",
        codeVerifier: "verifier",
