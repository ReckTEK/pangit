...
import type { GiteaReleaseEntityPayload } from "../native/GiteaReleaseNative.ts";

export type AnyGiteaRelease = GiteaReleaseEntityPayload<GiteaVersion, "release">;

export type AnyGiteaReleaseAsset = GiteaReleaseEntityPayload<GiteaVersion, "releaseAsset">;

export function requireReleaseArray<TVersion extends GiteaVersion>(
...
export function requireAssetArray<TVersion extends GiteaVersion>(
  context: GiteaAdapterContext<TVersion>,
  operation: string,
  response: AnyRestResponse,
...
export function isReleasePayload(value: unknown): value is AnyGiteaRelease {
  return isRecord(value) && hasText(value.id) && hasText(value.tag_name);
}

export function isAssetPayload(value: unknown): value is AnyGiteaReleaseAsset {
