...
async function publicTypes() {
  const first = await createClient("gitlab", "18.11.11", "https://gitlab.invalid");
  const second = await createClient("gitlab", "19.3.1", "https://gitlab.invalid");
  ...
  await createClient("gitlab", "1.27.2", "https://gitlab.invalid");
