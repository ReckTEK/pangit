...
Deno.test("README results are ordered, repeatable, and preserve human content without runtime noise", async () => {
  await withFixture(async (root) => {
    ...
    assert(
      first.includes("<!-- @generated by codegen/generate.ts. DO NOT EDIT. -->"),
      "Missing section-scoped generated warning",
    );
...
Deno.test("workspace README can live outside the package and links to package-owned reports", async () => {
  await withFixture(async (root) => {
    const packageRoot = new URL("packages/pangit/", root);
    await snapshot(packageRoot);
    await generateReadme(packageRoot, {
      readmePath: new URL("README.md", root),
      reportPrefix: "packages/pangit/",
    });
    const readme = await Deno.readTextFile(new URL("README.md", root));
    assert(readme.startsWith(before) && readme.endsWith(after), "Human README content changed");
    assert(
      readme.includes("[1.0.0](packages/pangit/docs/test-results/gitea/1.0.0/test-result.md)"),
      "Package-relative report link is missing",
    );
  });
});
