/** Provider manifests and reviewed public names define the generated clients' stable API. */
import { apiSpecProviders } from "../specs/sources.ts";
import { providerClientArtifact } from "../provider-layout.ts";
...
export async function readSpecManifest(file: URL): Promise<RestClientSpecManifest> {
  ...
  for (const [provider, providerValue] of objectEntries(providers)) {
    ...
    for (const [version, versionValue] of objectEntries(versions)) {
      ...
      const expectedClient = providerClientArtifact(provider, version);
      if (
        artifacts === undefined || artifacts.client !== expectedClient ||
        !artifacts.normalized?.startsWith("codegen/pangit/specs/normalized/")
...
export function assertPublicNamesCurrent(
  ...
  throw new Error(
    "Generated public names differ from codegen/pangit/generator/public-names.json; review the API change, then use deno task generate --update-public-names",
  );
}
