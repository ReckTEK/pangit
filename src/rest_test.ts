...
Deno.test("RestClient serializes generated requests with native Fetch types", async () => {
  ...
  const result = await client.request<RestResponse<200, { id: number }, "application/json", true>>(
    operation,
    {
      path: { owner: "branch press", path: "docs/read me.md" },
  ...
  assertEquals(
    captured.url,
    "https://example.test/api/v1/repos/branch%20press/docs/read%20me.md?api_version=1&labels=bug%2Chelp+wanted",
  );
...
Deno.test("RestClient matches parameterized response media by essence", async () => {
  let captured: Request | undefined;
  const jsonMediaType = "Application/Vnd.Branch-Press+JSON; version=1";
  const textMediaType = "Text/Plain; charset=UTF-8";
  ...
  const parameterizedOperation: RestOperation = {
    ...
    responses: [{
      ...
      decoders: {
        ...
        "text/plain": "text",
        "application/vnd.branch-press+json": "json",
      },
    }],
  };
  const client = new RestClient({
    ...
    fetch: (input) => {
      ...
      return Promise.resolve(
        new Response('{"id":42}', {
          headers: {
            "content-type": "APPLICATION/VND.BRANCH-PRESS+JSON; charset=UTF-8",
          },
        }),
      );
    },
...
Deno.test("RestClient matches a response before using generated decode metadata", async () => {
  const decodedOperation: RestOperation = {
    ...
    responses: [{
      status: 200,
      mediaTypes: ["application/vnd.branch-press.fixture"],
      decoders: { "application/vnd.branch-press.fixture": "json" },
    }],
  };
  const client = new RestClient({
    baseUrl: "https://example.test",
    fetch: () =>
      Promise.resolve(
        new Response('{"id":42}', {
          headers: { "content-type": "application/vnd.branch-press.fixture" },
        }),
      ),
  });

  const result = await client.request<
    RestResponse<200, { id: number }, "application/vnd.branch-press.fixture", true>
  >(decodedOperation);
...
Deno.test("RestClient matches default response metadata for an unlisted status", async () => {
  const defaultOperation: RestOperation = {
    ...
    responses: [
      ...
      {
        status: "default",
        mediaTypes: ["application/vnd.branch-press.error"],
        decoders: { "application/vnd.branch-press.error": "json" },
      },
    ],
  };
  const client = new RestClient({
    ...
    fetch: () =>
      Promise.resolve(
        new Response('{"message":"teapot"}', {
          status: 418,
          headers: { "content-type": "application/vnd.branch-press.error" },
        }),
      ),
  });
  const result = await client.request<
    RestResponse<
      number,
      { message: string },
      "application/vnd.branch-press.error",
      boolean
    >
  >(defaultOperation);
